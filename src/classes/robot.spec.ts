...
describe('Robot', () => {
  ...
  test(`should report the position correctly`, () => {
    ...
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y, FaceType.NORTH)}`);
    robot.input(new CommandLeft(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y, FaceType.WEST)}`);
    robot.input(new CommandLeft(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y, FaceType.SOUTH)}`);
    robot.input(new CommandLeft(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y, FaceType.EAST)}`);
    robot.input(new CommandLeft(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y, FaceType.NORTH)}`);
    ...
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y + 1, FaceType.NORTH)}`);
    robot.input(new CommandPlace(`${x}, ${y}, ${FaceType.WEST}`));
    robot.input(new CommandMove(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x - 1, y, FaceType.WEST)}`);
    robot.input(new CommandPlace(`${x}, ${y}, ${FaceType.SOUTH}`));
    robot.input(new CommandMove(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x, y - 1, FaceType.SOUTH)}`);
    robot.input(new CommandPlace(`${x}, ${y}, ${FaceType.EAST}`));
    robot.input(new CommandMove(''));
    robot.input(new CommandReport(''));
    expect(consoleSpy).toHaveBeenLastCalledWith(`${formatOutput(x + 1, y, FaceType.EAST)}`);
  });
});
