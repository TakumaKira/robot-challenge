...
import { TABLE_DEPTH, TABLE_WIDTH } from '../config.json';
...
describe('CommandProcessor', () => {
  ...
  test(`should return null if given invalid command`, () => {
    ...
    expect(processCommand(`${CommandType.PLACE} ${TABLE_WIDTH},0,${FaceType.NORTH}`)).toBe(null);
    expect(processCommand(`${CommandType.PLACE} 0,${TABLE_DEPTH},${FaceType.NORTH}`)).toBe(null);
    expect(processCommand(`${CommandType.PLACE} 0,0,0,${FaceType.NORTH}`)).toBe(null);
